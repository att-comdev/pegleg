...
@click.option(
    ...
    default=sys.stdout,
    help='Where to output the complete config. Must end in /site.')
@click.option(
    '--no-validate',
    'no_validate',
    is_flag=True,
...
def collect(*, save_location, no_validate, exclude_lint, warn_lint, site_name):
    if not save_location.endswith('/site'):
        raise click.ClickException('save_location must end in /site.')
    ...
    if not no_validate:
        ...
        warns = engine.lint.full(fail_on_missing_sub_src=True,
                                 warn_lint=warn_lint,
