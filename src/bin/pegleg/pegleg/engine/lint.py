...
def full(fail_on_missing_sub_src=False, exclude_lint=None, warn_lint=None):
    ...
    for msg in messages:
        if (SCHEMA_STORAGE_POLICY_MISMATCH_FLAG in warn_lint
                and SCHEMA_STORAGE_POLICY_MISMATCH_FLAG in msg[0]):
            warns.append(msg)
        else:
            errors.append(msg)
    ...
    if DECKHAND_RENDERING_INCOMPLETE_FLAG in warn_lint:
        ...
        errors.extend([(DECKHAND_RENDERING_INCOMPLETE_FLAG, x) for x in _verify_deckhand_render(fail_on_missing_sub_src)])
    ...
    if REPOS_MISSING_DIRECTORIES_FLAG in warn_lint:
        ...
        errors.extend([(REPOS_MISSING_DIRECTORIES_FLAG, x) for x in _verify_no_unexpected_files()])
...
def _verify_document(document, schemas, filename):
    ...
    if document.get('schema') in MANDATORY_ENCRYPTED_TYPES:
        ...
        if not _filename_in_section(filename, 'secrets/'):
            errors.append(
                ('N/A', '%s (document %s) is a secret, is not stored in a '
                    'secrets path'
                % (filename, name)))
    return errors
