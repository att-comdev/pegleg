...
def create_tmp_deployment_files(tmpdir):
    """Fixture that creates a temporary directory structure that looks like:
...
""" % (revision, site_type, site_type)
    ...
    t = p.mkdir("type")
    # Create type/cicd and type/lab directories and sub-directories.
    for site_type in site_types:
        t1 = t.mkdir(site_type)
        t1.mkdir("common").join("type-common.yaml").write(
            "fake-type-common-%s" % site_type)
        t1.mkdir(revision).join("type-%s.yaml" % revision).write(
            "fake-type-%s" % revision)

    # Create site/ directory.
    s = p.mkdir("site")
    # Create site/cicd and site/lab directories and sub-directories.
